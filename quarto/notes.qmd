...
- [x] Port code I already have to this repo.
- [x] Identify the relevant bits that have to come in here.
...
- [-] Remove having to load pickles.
    - Can't be done without going to jail.
...
- [x] Introduce PyGT and PyG libraries.
...
- [-] Find a better way to distribute the adjacency matrix than a pickle.
    - Licensing will be an issue, so pickle it is.
...
- [x] Implement Diffusion conv with the correct normalisation.
...
- [x] Implement adjacency precomputation per forward pass
- [x] Implement adaptive adjacency computation per forward pass using ```torch.block_diag```.
...
- [x] Introduce torch lightning (talk).
...
- [x] Debug/optimise the adaptive adjacency.
...
Might as well try to reproduce the results now.
Running it with the adjacency matrices will take ages.
In any case, there should be everything to attempt a faithful reproduction.

To do the reproduction, I need an easy way to split the dataset into train/test/val.

- [x] Implement a split flag into the METRLA dataset.
    - Actually, easier to do manually after loading.
